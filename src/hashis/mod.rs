...
pub fn gcrc128(data: &[u8]) -> [u8; 16] {
    ...
    let mut we64: [u8; 16] = *b"0000000000000000";
    let prewe64 = format!("{:x}", WE_64.checksum(&data)).as_bytes().to_vec();
    let modwe64 = prewe64.len() % 16;
    if modwe64 > 0 {
        we64[..modwe64].copy_from_slice(&prewe64);
    } else {
    ...
    if modgo64 > 0 {
        ...
        go64.copy_from_slice(&prego64);
    }

    let mut lhs = hex::decode(we64).expect("failed to CRC64_WE");
    lhs.reverse();
    let rhs = hex::decode(go64).expect("failed to CRC64_GO");
