...
e2e: cleanx debug test
	...
	echo "Hello World from STDIN" | $(OBG_RUN) encrypt text -k $(OBG_KEY0) > cipher.txt
	test "$$($(OBG_RUN) decrypt text -k $(OBG_KEY0) "$$(cat cipher.txt)")" = "Hello World from STDIN"
	test "$$(cat cipher.txt | $(OBG_RUN) decrypt text -k $(OBG_KEY0))" = "Hello World from STDIN"
	...
	dd if=/dev/random of=$(OBG_KEY0) bs=8 count=1024
